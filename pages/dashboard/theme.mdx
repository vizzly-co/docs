...
The Vizzly dashboard exposes multiple components that can be replaced.
For example, you can provide a custom loading component using the `renderLoadingComponent` React prop.
...
To see all available components that are replaceable, [see the overrides documentation](./overrides)
