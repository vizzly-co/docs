...
The following are examples of a Vizzly config file for various integrations;
...
Please note, that the `table` field refers to an elastic search `index`.

```json
{
  "connection": {
    "client": "elasticsearch",
    "node": "https://my-elastic-search-endpoint.com",
    "auth": {
      "apiKey": "<< a secret >>"
    }
  },
...
After uploading the `vizzly.config.json` file securely to an S3 bucket, specify the `VIZZLY_CONFIG_S3_BUCKET_NAME` and `VIZZLY_CONFIG_S3_BUCKET_REGION` environment variables.
...
As this config file will contain read-only database credentials, be sure to store this securely in S3.
...
Remember to set the `keyFilename` value in the BigQuery connection to the same location as where you've mounted the service account config JSON file.
